...
class PropagationModule(nn.Module):
    ...
    def img2col(self, x):
        n, c, w, h = x.size()
        feature_pad = F.pad(x, (self.kernel // 2, self.kernel // 2, self.kernel // 2, self.kernel // 2), "constant",
                            value=0)
        img2col = feature_pad.unfold(2, self.kernel, 1).unfold(3, self.kernel, 1).contiguous().view(n, c, w * h,
                                                                                                    self.kernel ** 2)
        return img2col
