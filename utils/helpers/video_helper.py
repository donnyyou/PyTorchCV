...
from utils.tools.progressbar import track_progress
...
class VideoHelper(object):
    ...
    def frames2video(frame_dir,
        ...
        if end == 0:
            ...
            end = len([name for name in FileHelper.scandir(frame_dir, ext)])
        first_file = osp.join(frame_dir, filename_tmpl.format(start))
        FileHelper.check_file_exist(first_file, 'The start frame not found: ' + first_file)
        img = cv2.imread(first_file)
        height, width = img.shape[:2]
        resolution = (width, height)
