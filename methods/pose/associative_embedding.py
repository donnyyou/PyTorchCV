...
class AssociativeEmbedding(object):
    """
    ...
    def init_model(self):
        ...
        self.embedding_loss = self.pose_loss_manager.get_pose_loss('embedding_loss')
    ...
    def __train(self):
        ...
        for i, (inputs, heatmap, maskmap, tagmap, num_objects) in enumerate(self.train_loader):
            self.data_time.update(time.time() - start_time)
            # Change the data type.
            (inputs, heatmap, maskmap,
             tagmap, num_objects) = self.module_utilizer.to_device(inputs, heatmap, maskmap, tagmap, num_objects)

            # Forward pass.
            embed_out, heatmap_out = self.pose_net(inputs)
            # Compute the loss of the train batch & backward.
            loss_heatmap = self.mse_loss(heatmap_out, heatmap, maskmap)
            loss_associate = self.embedding_loss(embed_out, tagmap, num_objects)
    ...
    def __val(self):
        ...
        with torch.no_grad():
            for j, (inputs, heatmap, maskmap, tagmap, num_objects) in enumerate(self.val_loader):
                # Change the data type.
                (inputs, heatmap, maskmap,
                 tagmap, num_objects) = self.module_utilizer.to_device(inputs, heatmap, maskmap, tagmap, num_objects)
            ...
            Log.info('Loss Heatmap:{}, Loss Asso: {}'.format(self.val_loss_heatmap.avg,
