        ...
        for i, anchor_size in enumerate(self.configer.get('gt', 'cur_anchor_sizes')):
            ...
            for b in range(batch_size):
                for t in range(batch_gt_bboxes[b].size(0)):
                    ...
                    grid_x = (batch_gt_bboxes[b][t, 0] + batch_gt_bboxes[b][t, 2]) / (2.0 * input_size[0]) * in_w
                    grid_y = (batch_gt_bboxes[b][t, 1] + batch_gt_bboxes[b][t, 3]) / (2.0 * input_size[1]) * in_h
                    gw = (batch_gt_bboxes[b][t, 2] - batch_gt_bboxes[b][t, 0])
                    gh = (batch_gt_bboxes[b][t, 3] - batch_gt_bboxes[b][t, 1])
                    if gw * gh == 0 or grid_x >= in_w or grid_y >= in_h:
                        continue

                    if gw > anchor_size * 2 or gh > anchor_size * 2 or gw < anchor_size / 2 or gh < anchor_size / 2:
                        continue
                    ...
                    pred_ious = DetHelper.bbox_iou(pred_box, gt_box)
                    best_n = torch.argmax(pred_ious, dim=0)
