...
import warnings
import numpy as np
import extensions.utils.det.src.cython_bbox as cython_bbox


class BBoxesHelper(object):
    ...
    @staticmethod
    def unique_bboxes(boxes, scale=1.0):
    ...
    @staticmethod
    def filter_small_bboxes(boxes, min_size):
    ...
    @staticmethod
    def clip_bboxes_to_image(boxes, height, width):
    ...
    @staticmethod
    def clip_tiled_bboxes(bboxes, im_shape):
        ...
        # x1 >= 0
        bboxes[:, 0::4] = np.maximum(np.minimum(bboxes[:, 0::4], im_shape[1] - 1), 0)
        # y1 >= 0
        bboxes[:, 1::4] = np.maximum(np.minimum(bboxes[:, 1::4], im_shape[0] - 1), 0)
        # x2 < im_shape[1]
        bboxes[:, 2::4] = np.maximum(np.minimum(bboxes[:, 2::4], im_shape[1] - 1), 0)
        # y2 < im_shape[0]
        bboxes[:, 3::4] = np.maximum(np.minimum(bboxes[:, 3::4], im_shape[0] - 1), 0)
        return bboxes

    @staticmethod
    def bbox_transform(bboxes, deltas, weights=(1.0, 1.0, 1.0, 1.0)):
    ...
    def box_voting(top_dets, all_dets, thresh, scoring_method='ID', beta=1.0):
        ...
        top_to_all_overlaps = cython_bbox.bbox_overlaps(top_boxes, all_boxes)
