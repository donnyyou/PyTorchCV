...
print('compiling/loading encoding')
build_path = '/tmp/bulid/encoding'
if not os.path.exists(build_path):
    os.makedirs(build_path)

encoding = load(name='encoding', sources=['extensions/layers/encoding/src/operator.cpp',
                                          'extensions/layers/encoding/src/encoding_kernel.cu'],
...
class _aggregate(Function):
    ...
    def forward(ctx, A, X, C):
        ...
        if A.is_cuda:
            E = encoding.aggregate_forward(A, X, C)
        else:
    ...
    def backward(ctx, gradE):
        ...
        if A.is_cuda:
            gradA, gradX, gradC = encoding.aggregate_backward(gradE, A, X, C)
        else:
            raise NotImplemented
...
def aggregate(A, X, C):
    ...
    Examples:
        >>> B,N,K,D = 2,3,4,5
        >>> A = torch.cuda.DoubleTensor(B,N,K).uniform_(-0.5,0.5)
...
class _scaledL2(Function):
    ...
    def forward(ctx, X, C, S):
        if X.is_cuda:
            SL = encoding.scaled_l2_forward(X, C, S)
        else:
            raise NotImplemented
    ...
    def backward(ctx, gradSL):
        ...
        if X.is_cuda:
            gradX, gradC, gradS = encoding.scaled_l2_backward(gradSL, X, C, S, SL)
        else:
            raise NotImplemented
